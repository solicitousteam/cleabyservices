...
if(isset($_POST['form1'])) {
    if(empty($_POST['email']) && empty($_POST['password'])) {
        ...
        echo
		    '<script>
		        alert("Email and/or Password can not be empty");  window.location="login.php";
		    </script>';
		    die;
    } else {
		...
		$row = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM tbl_user WHERE email='".$_POST['email']."' AND password='".md5($_POST['password'])."'"));
		if(empty($row))
		{
		    echo
		    '<script>
